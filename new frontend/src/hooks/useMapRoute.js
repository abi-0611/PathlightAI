...
export function useMapRoute(mapRef, routes, activeRouteId, isNight) {
  ...
  useEffect(() => {
      ...
      routes.forEach((route) => {
        ...
        if (isActive) {
          map.addLayer({
            ...
            paint: { 'line-color': color, 'line-width': 12, 'line-opacity': 0.2, 'line-blur': 10 }
          });
    ...
    // If style already loaded, draw immediately; otherwise wait
    ...
    // Re-draw after style swaps (day/night toggle clears all layers)
