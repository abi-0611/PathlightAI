...
export default function App() {
  ...
  // New hooks
  useParallax({ intensity: 5 });
  const { notifications, addNotification, dismissNotification } = useNotifications();
  const bottomSheet = useBottomSheet(false);
  const [mobileActiveTab, setMobileActiveTab] = useState('vibe');
  ...
  useEffect(() => {
    ...
    map.on('error', (e) => {
      ...
      if (e.error?.status === 401 || msg.includes('Unauthorized') || msg.includes('401')) {
        addNotification('error', 'Mapbox token is invalid or expired. Check VITE_MAPBOX_KEY in .env');
      }
  ...
  useEffect(() => {
    ...
    const fetchHeatmap = async () => {
      try {
        ...
        addNotification('warning', 'Failed to load danger heatmap');
      }
    };
    fetchHeatmap();
  ...
  useEffect(() => {
    if (gpsError) addNotification('warning', `GPS: ${gpsError}`);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [gpsError]);
  ...
  // Route fetch errors → notification
  useEffect(() => {
    if (!error) return;
  ...
  // Parallax handled by useParallax() hook above
  ...
  return (
    <div className="relative w-full h-full bg-obsidian-950 grain">
      ...
      {/* Map Toolbar — replaces the 4 individual icon buttons */}
      <MapToolbar
        gpsPosition={gpsPosition}
        destination={destination}
        routeGeoJSON={routeGeoJSON}
        arrived={arrived}
        isNavFollowing={isNavFollowing}
        ...
        onToggleFollow={() => setIsNavFollowing((v) => !v)}
      ...
      {/* MOBILE: BottomSheet with Vibe + Insights tabs */}
      <div className="sm:hidden">
        {!bottomSheet.isOpen && (
          <button
            onClick={bottomSheet.open}
            className="fixed bottom-6 left-1/2 -translate-x-1/2 z-20 glass px-5 py-2.5
                       flex items-center gap-2 text-sm font-body"
            style={{ color: 'var(--c-text-1)' }}
          >
            <span
              className="w-2 h-2 rounded-full"
              style={{ background: 'var(--c-teal)', animation: 'ping 1.5s cubic-bezier(0,0,0.2,1) infinite' }}
            />
            Route Settings
          </button>
        )}
        ...
        <BottomSheet
          ...
          dragY={bottomSheet.dragY}
          isDragging={bottomSheet.isDragging}
          touchHandlers={bottomSheet.touchHandlers}
          title={null}
          maxHeightClass="max-h-[72vh]"
        >
          {/* Tab switcher */}
          <div
            className="flex gap-1 p-1 rounded-xl mx-1 mb-3"
            style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(255,255,255,0.05)' }}
          >
            <button
              onClick={() => setMobileActiveTab('vibe')}
              className="flex-1 py-2 rounded-lg text-xs font-body font-medium transition-all duration-200"
              style={mobileActiveTab === 'vibe'
                ? { background: 'var(--c-teal-dim)', color: 'var(--c-teal)', border: '1px solid var(--c-border-act)' }
                : { color: 'var(--c-text-3)' }}
            >
              🎛 Route DNA
            </button>
            <button
              ...
              className="flex-1 py-2 rounded-lg text-xs font-body font-medium transition-all duration-200"
              style={mobileActiveTab === 'insights'
                ? { background: 'var(--c-teal-dim)', color: 'var(--c-teal)', border: '1px solid var(--c-border-act)' }
                : { color: 'var(--c-text-3)' }}
            >
              ✨ Insights
      ...
      {/* Notification Stack — replaces ErrorToast */}
      <NotificationStack
